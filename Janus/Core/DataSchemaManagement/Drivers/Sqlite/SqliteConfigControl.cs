...
namespace Rsdn.Janus
{
	public partial class SqliteConfigControl: DBConfigControlBase
	{
		...
		{
			List<string> existDatabasePathList = new List<string>();
			foreach (string path in ConfigSelDb.Instance.SqliteDatabasePathList)
			{
				if (File.Exists(path))
			...
			if (localize)
			{
				_selectDatabasePathLabel.Text = SR.Database.ConfigControls.Sqlite.DatabasePath;
				_selectDatabasePathOpenFileDialog.Filter = SR.Database.ConfigControls.Sqlite.OpenFileDialogFilter;
			}
		}
