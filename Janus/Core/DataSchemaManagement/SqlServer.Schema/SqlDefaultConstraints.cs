...
{
	internal class SqlDefaultConstraints : SqlDbSchema
	{
		protected override SqlClause GetCommandText(object[] restrictions)
		{
			SqlClause sql = new SqlClause();

			DataRow[] col = _sqlClauses.Select("CollectionName = 'DefaultConstraints' and Version=" + _version.ver1);
		...
		protected override DataTable ProcessResult(DataTable schema)
		{
			...
			foreach (DataRow row in schema.Rows)
			{
				...
				int helperCid = Convert.ToInt32(row["HELPER_CID"], CultureInfo.InvariantCulture);
				if (helperCid != 0)
					row["SOURCE"] = HelperGetObjectDefinition(helperCid);
			}
