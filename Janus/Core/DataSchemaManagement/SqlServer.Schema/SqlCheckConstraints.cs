...
namespace Rsdn.Janus
{
	internal class SqlCheckConstraints : SqlDbSchema
	{
		protected override SqlClause GetCommandText(object[] restrictions)
		{
			...
			DataRow[] col = _sqlClauses.Select("CollectionName = 'CheckConstraints' and Version=" + _version.ver1);
		...
		protected override DataTable ProcessResult(DataTable schema)
		{
			...
			foreach (DataRow row in schema.Rows)
			{
				...
				{
					row["SOURCE"] = HelperGetObjectDefinition(Convert.ToInt32(row["HELPER_CID"], CultureInfo.InvariantCulture));
				}
			}
